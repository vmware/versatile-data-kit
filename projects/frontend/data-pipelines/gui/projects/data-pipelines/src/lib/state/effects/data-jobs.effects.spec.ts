...
describe('DataJobsEffects', () => {
    ...
    beforeEach(waitForAsync(() => {
        dataJobsApiServiceStub = jasmine.createSpyObj<DataJobsApiService>('dataJobsApiService', [
            'getJobs', 'getJob', 'getJobDetails', 'getJobExecutions', 'updateDataJob', 'updateDataJobStatus'
        ]);
    ...
    describe('Effects::', () => {
        describe('|loadDataJobs$|', () => {
            it('should verify will load data-jobs', marbles((m) => {
                // Given
                actions$ = m.cold('-a---------b', {
        ...
        describe('|loadDataJob$|', () => {
            it('should verify will load data-job', marbles((m) => {
                // Given
                actions$ = m.cold('-a----------', {
                ...
                componentServiceStub.getModel.and.callFake(() => {
                    ...
                    switch (cntComponentServiceStub) {
                        ...
                        case 4:
                            return m.cold('---a', {
                                a: createModel(
                                    new Map<string, any>([getJobStateTuple(), getDataJobDetailsTuple()]),
                                    1,
                                    LOADED
                                )
                            });
        ...
        describe('|loadDataJobExecutions$|', () => {
            it('should verify will load data-job executions', marbles((m) => {
                // Given
        ...
        describe('|updateDataJob$|', () => {
            it('should verify will update data-job description', marbles((m) => {
                // Given
                ...
                componentServiceStub.getModel.and.callFake(() => m.cold('---a', {
            ...
            it('should verify will update data-job status', marbles((m) => {
                // Given
                const genericAction1 = GenericAction.of(
                    UPDATE_DATA_JOB,
                    createModel(undefined, 1, LOADING).getComponentState(),
            ...
            it('should verify will fail', marbles((m) => {
                // Given
                const genericAction1 = GenericAction.of(
                    UPDATE_DATA_JOB,
                    createModel(undefined, 1, LOADING).getComponentState(),
                ...
                const expected$ = m.cold('----a-', {
                    a: ComponentFailed.of(
                        createModel()
                            .withTask(genericAction1.task)
                            .withError(new Error('Unsupported ComponentModel update task'))
                            .withStatusFailed()
                            .getComponentState()
                    )
                });
...
const createModel = (data?: Map<string, any>,
                     navigationId = 1,
                     status: StatusType = LOADING,
                     requestParams: Array<[string, any]> = []) => ComponentModel.of(
