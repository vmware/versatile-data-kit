...
export class DataJobsApiService extends TaurusBaseApiService<DataJobsApiService> {
  ...
  constructor(
    ...
    private readonly dataPipelinesConfig: DataPipelinesConfig,
    ...
    if (this.dataPipelinesConfig?.ownerTeamNamesObservable) {
      this.dataPipelinesConfig.ownerTeamNamesObservable.subscribe(
        (result: string[]) => {
          if (result?.length) {
  ...
  downloadFile(teamName: string, jobName: string): Observable<Blob> {
    ...
    return this.http.get(
      `${this._resolvePipelinesServiceUrl()}/data-jobs/for-team/${teamName}/jobs/${jobName}/keytab`,
      {
        headers: httpHeaders,
        responseType: "blob",
      },
  ...
  private _resolvePipelinesServiceUrl(): string {
    return this.dataPipelinesConfig?.resourceServer?.getUrl
      ? this.dataPipelinesConfig.resourceServer.getUrl()
      : "";
  }
  ...
  private _validateModuleConfig(
    dataPipelinesConfig: DataPipelinesConfig,
  ): void {
