...
describe('Data Job Explore Executions Page', { tags: ['@dataPipelines', '@exploreDataJobExecutions'] }, () => {
    ...
    it(`Data Job Explore Executions Page - should open Details and verify Executions tab is not displayed`, () => {
        ...
        dataJobBasePage.getMainTitle().should('be.visible').should('contains.text', testJobs[0].job_name);
    ...
    it('Data Job Explore Executions Page - should verify on URL navigate to Executions will redirect to Details', () => {
        const dataJobBasePage = DataJobBasePO.navigateToUrl(`/explore/data-jobs/${testJobs[0].team}/${testJobs[0].job_name}/executions`);

        dataJobBasePage.getMainTitle().should('be.visible').should('contains.text', testJobs[0].job_name);

        dataJobBasePage.getCurrentUrl().should('match', new RegExp(`\\/explore\\/data-jobs\\/${testJobs[0].team}\\/${testJobs[0].job_name}\\/details$`));
