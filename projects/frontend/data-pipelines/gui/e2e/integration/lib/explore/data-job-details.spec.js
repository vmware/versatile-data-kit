...
describe('Data Job Explore Details Page', { tags: ['@dataPipelines', '@exploreDataJobDetails'] }, () => {
    ...
    it('Data Job Explore Details Page - should load and show job details', () => {
        ...
        dataJobsExplorePage.openJobDetails(testJobs[0].team, testJobs[0].job_name);

        dataJobExploreDetailsPage = DataJobExploreDetailsPage.getPage();

        dataJobExploreDetailsPage.getDetailsTab().should('be.visible');

        dataJobExploreDetailsPage.getMainTitle().should('be.visible').should('contains.text', testJobs[0].job_name);

        dataJobExploreDetailsPage.getStatusField().should('be.visible').should('have.text', 'Not Deployed');

        dataJobExploreDetailsPage.getDescriptionField().should('be.visible').should('contain.text', testJobs[0].description);

        dataJobExploreDetailsPage.getTeamField().should('be.visible').should('have.text', testJobs[0].team);

        dataJobExploreDetailsPage.getScheduleField().should('be.visible').should('contains.text', 'At 12:00 AM, on day 01 of the month, and on Friday');
        ...
        dataJobExploreDetailsPage.getOnDeployedField().should('be.visible').should('contains.text', testJobs[0].config.contacts.notified_on_job_deploy);

        dataJobExploreDetailsPage.getOnPlatformErrorField().should('be.visible').should('contains.text', testJobs[0].config.contacts.notified_on_job_failure_platform_error);

        dataJobExploreDetailsPage.getOnUserErrorField().should('be.visible').should('contains.text', testJobs[0].config.contacts.notified_on_job_failure_user_error);

        dataJobExploreDetailsPage.getOnSuccessField().should('be.visible').should('contains.text', testJobs[0].config.contacts.notified_on_job_success);
    });
    ...
    it('Data Job Explore Details Page - should verify Details tab is visible and active', () => {
        ...
        dataJobExploreDetailsPage.getDetailsTab().should('be.visible').should('have.class', 'active');
    });
