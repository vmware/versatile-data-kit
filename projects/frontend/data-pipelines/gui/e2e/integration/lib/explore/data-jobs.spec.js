...
describe('Data Jobs Explore Page', { tags: ['@dataPipelines', '@exploreDataJobs'] }, () => {
    ...
    it('Main Title Component have text: Explore Data Jobs', () => {
        ...
        dataJobsExplorePage
            .getMainTitle()
            .should('be.visible')
            .should(($el) => expect($el.text().trim()).to.equal('Explore Data Jobs'));
    });
    ...
    it('Data Jobs Explore Page - loaded and shows data jobs', () => {
        ...
        testJobs.forEach((testJob) => {
            ...
            dataJobsExplorePage.getDataGridCell(testJob.team).should('be.visible');
        });
    });
    ...
    it('Data Jobs Explore Page - filters data jobs', () => {
        ...
        dataJobsExplorePage.filterByJobName(testJobs[0].job_name);

        dataJobsExplorePage.getDataGridCell(testJobs[0].job_name).should('be.visible');

        dataJobsExplorePage.getDataGridCell(testJobs[1].job_name).should('not.exist');
    });
    ...
    it('Data Jobs Explore Page - refreshes data jobs', () => {
        ...
        cy.fixture('lib/explore/additional-test-job.json').then((additionalTestJob) => {
            ...
            dataJobsExplorePage.getDataGridCell(testJobs[0].job_name).should('have.text', testJobs[0].job_name);

            dataJobsExplorePage.getDataGridCell(normalizedTestJob.job_name).should('not.exist');

            cy.prepareAdditionalTestJobs();

            dataJobsExplorePage.refreshDataGrid();

            dataJobsExplorePage.filterByJobName(normalizedTestJob.job_name);

            dataJobsExplorePage.getDataGridCell(normalizedTestJob.job_name).should('have.text', normalizedTestJob.job_name);
        });
    });
