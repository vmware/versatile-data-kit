...
describe('Data Job Manage Details Page', { tags: ['@dataPipelines', '@manageDataJobDetails'] }, () => {
    ...
    it('Data Job Manage Details Page - disable/enable job', { tags: '@integration' }, () => {
        dataJobManageDetailsPage = DataJobManageDetailsPage.navigateTo(longLivedTestJob.team, longLivedTestJob.job_name);
    ...
    it('Data Job Manage Details Page - edit job description', { tags: '@integration' }, () => {
        ...
        dataJobManageDetailsPage = DataJobManageDetailsPage.navigateTo(testJobs[0].team, testJobs[0].job_name);
        ...
        dataJobManageDetailsPage.getDescription().should('be.visible').should('contain.text', newDescription.split(' ').slice(0, descriptionWordsBeforeTruncate).join(' '));
    });
    ...
    it('Data Job Manage Details Page - download job key', () => {
        ...
        dataJobManageDetailsPage.readFile('downloadsFolder', `${longLivedTestJob.job_name}.keytab`).should('exist');
    });
    ...
    it('Data Job Manage Details Page - delete job', { tags: '@integration' }, () => {
        cy.fixture('lib/explore/additional-test-job.json').then((additionalTestJob) => {
            ...
            dataJobManageDetailsPage = DataJobManageDetailsPage.navigateTo(normalizedTestJob.team, normalizedTestJob.job_name);
    ...
    it.skip('Data Job Manage Details Page - executions timeline', () => {
        ...
        dataJobManageDetailsPage = DataJobManageDetailsPage.navigateTo(team, jobName);

        cy.wait('@executionApiCall').then((interception) => {
            const response = interception.response.body;
            const lastExecutions = response.sort((left, right) => compareDatesAsc(left, right)).slice(response.length > 5 ? response.length - 5 : 0);

            const lastExecutionsSize = lastExecutions.length;
            const lastExecution = lastExecutions.at(-1);

            cy.get('.clr-timeline-step').should('have.length', lastExecutionsSize + 1); // +1 next execution
            ...
            if (lastExecution.type !== 'manual') {
                cy.get('@lastExecution').find('.manual-execution-label').scrollIntoView().should('be.visible');
            }

            if (lastExecution.status !== 'running') {
                cy.get('@lastExecution').find(`[shape=${statusIconMap[lastExecution.status]}]`).should('exist');
            }

            cy.get('@lastExecution').find('.clr-timeline-step-header').invoke('attr', 'title').should('contain', 'Started ');

            if (lastExecution.status !== 'running') {
                cy.get('@lastExecution').find('u').last().invoke('attr', 'title').should('contain', 'Ended ');
            }
        });
    });
});
