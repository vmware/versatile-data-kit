...
describe('Data Job Manage Executions Page', { tags: ['@dataPipelines', '@manageDataJobExecutions'] }, () => {
    ...
    describe('Sanity', { tags: '@integration' }, () => {
        it(`Data Job Manage Executions Page - should open Details and verify Executions tab is displayed and navigates`, () => {
            ...
            dataJobsManagePage.openJobDetails(longLivedTestJob.team, longLivedTestJob.job_name);

            const dataJobBasePage = DataJobBasePO.getPage();

            dataJobBasePage.getDetailsTab().should('exist').should('have.class', 'active');

            dataJobBasePage.getExecutionsTab().should('exist').should('not.have.class', 'active');
        ...
        it('Data Job Manage Executions Page - should verify on URL navigate to Executions will open the page', () => {
            const dataJobExecutionsPage = DataJobManageExecutionsPage.navigateTo(longLivedTestJob.team, longLivedTestJob.job_name);

            dataJobExecutionsPage.getCurrentUrl().should('match', new RegExp(`\\/manage\\/data-jobs\\/${longLivedTestJob.team}\\/${longLivedTestJob.job_name}\\/executions$`));
        ...
        it('Data Job Manage Executions Page - should verify elements are rendered in DOM', () => {
            ...
            dataJobExecutionsPage.getMainTitle().should('be.visible').should('contains.text', longLivedTestJob.job_name);
        ...
        it('Data Job Manage Executions Page - should verify cancel execution button works properly', () => {
            ...
            dataJobExecutionsPage.getConfirmDialogButton().should('exist').click({ force: true });
            dataJobExecutionsPage.waitForPostExecutionCompletion();
            dataJobExecutionsPage.waitForDataJobStartExecute();

            dataJobExecutionsPage.waitForViewToRenderShort();
            ...
            dataJobExecutionsPage.getCancelExecutionButton().should('exist').click({ force: true });
            dataJobExecutionsPage.getConfirmDialogButton().should('exist').click({ force: true });
