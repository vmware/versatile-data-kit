...
describe('Data Jobs Manage Page', { tags: ['@dataPipelines', '@manageDataJobs'] }, () => {
    ...
    before(() => {
        return DataJobsManagePage.recordHarIfSupported()
            ...
            .then(() =>
                cy.fixture('lib/explore/test-jobs.json').then((loadedTestJobs) => {
            ...
            .then(() =>
                cy.fixture('lib/manage/e2e-cypress-dp-test.json').then((loadedTestJob) => {
            ...
            .then(() =>
                cy.fixture('e2e-cy-dp-failing.job.json').then((failingTestJob) => {
    ...
    it('Data Jobs Manage Page - grid filter by job name', { tags: '@integration' }, () => {
        ...
        dataJobsManagePage.getDataGridCell(testJobs[0].job_name).should('be.visible');

        dataJobsManagePage.getDataGridCell(testJobs[1].job_name).should('not.exist');
    });
    ...
    it('Data Jobs Manage Page - grid search by job name', { tags: '@integration' }, () => {
        ...
        dataJobsManagePage.getDataGridCell(testJobs[0].job_name).should('not.exist');

        dataJobsManagePage.getDataGridCell(testJobs[1].job_name).should('be.visible');
    });
    ...
    it('Data Jobs Manage Page - grid search parameter goes into URL', () => {
        ...
        dataJobsManagePage.getDataGridCell(testJobs[0].job_name).scrollIntoView().should('be.visible');
        dataJobsManagePage.getDataGridCell(testJobs[1].job_name).scrollIntoView().should('be.visible');
        ...
        dataJobsManagePage.getCurrentUrl().should('match', new RegExp(`\\/manage\\/data-jobs\\?search=${testJobs[0].job_name}$`));
        ...
        dataJobsManagePage.getCurrentUrl().should('match', new RegExp(`\\/manage\\/data-jobs$`));
    });
    ...
    it('Data Jobs Manage Page - grid search perform search when URL contains search parameter', () => {
        ...
        // verify url contains search value
        dataJobsManagePage.getCurrentUrl().should('match', new RegExp(`\\/manage\\/data-jobs\\?search=${testJobs[1].job_name}$`));
    ...
    it('Data Jobs Manage Page - refresh shows newly created job', () => {
        cy.fixture('lib/explore/additional-test-job.json').then((additionalTestJob) => {
            ...
            dataJobsManagePage.getDataGridCell(testJobs[0].job_name).should('have.text', testJobs[0].job_name);

            dataJobsManagePage.getDataGridCell(normalizedTestJob.job_name).should('not.exist');

            dataJobsManagePage.prepareAdditionalTestJob();

            dataJobsManagePage.refreshDataGrid();

            dataJobsManagePage.filterByJobName(normalizedTestJob.job_name);

            dataJobsManagePage.getDataGridCell(normalizedTestJob.job_name).should('have.text', normalizedTestJob.job_name);
        });
    });
    ...
    it('Data Jobs Manage Page - click on edit button opens new page with Job details', { tags: '@integration' }, () => {
        ...
        dataJobsManagePage.openJobDetails(testJobs[0].team, testJobs[0].job_name);

        const dataJobManageDetailsPage = DataJobManageDetailsPage.getPage();

        dataJobManageDetailsPage.getMainTitle().should('be.visible').should('contains.text', testJobs[0].job_name);

        dataJobManageDetailsPage.getDescription().should('be.visible').should('contain.text', testJobs[0].description.split(' ').slice(0, descriptionWordsBeforeTruncate).join(' '));

        dataJobManageDetailsPage.getSchedule().should('be.visible');

        dataJobManageDetailsPage.getDeploymentStatus('not-deployed').should('be.visible').should('have.text', 'Not Deployed');
    });
    ...
    it('Data Jobs Manage Page - quick filters', { tags: '@integration' }, () => {
        ...
        DataJobsManagePage.changeJobStatus(longLivedFailingTestJob.team, longLivedFailingTestJob.job_name, true)
            .then(() => DataJobsManagePage.changeJobStatus(longLivedTestJob.team, longLivedTestJob.job_name, false))
            .then(() => {
                dataJobsManagePage.clickOnContentContainer();
