...
describe('Getting Started Page', { tags: ['@dataPipelines'] }, () => {
    ...
    it('Main Title Component have text: Get Started with Data Pipelines', () => {
        GettingStartedPage.navigateTo()
            .getMainTitle()
            .should(($el) => expect($el.text().trim()).to.equal('Get Started with Data Pipelines'));
    });
    ...
    describe('Data Jobs Health Overview Panel', () => {
        it('Verify Widgets rendered correct data and failing jobs navigates correctly', () => {
            ...
            let dataJobsHealthPanel = DataJobsHealthPanelComponentPO.getComponent();
            dataJobsHealthPanel.waitForViewToRender();
            dataJobsHealthPanel.getDataJobsHealthPanel().scrollIntoView();

            dataJobsHealthPanel.getExecutionsSuccessPercentage().should('be.gte', 0).should('be.lte', 100);
            dataJobsHealthPanel.getNumberOfFailedExecutions().should('be.gte', 2);
            dataJobsHealthPanel.getExecutionsTotal().should('be.gte', 2);

            dataJobsHealthPanel.getAllFailingJobs().should('have.length.gte', 1);

            dataJobsHealthPanel.getAllMostRecentFailingJobsExecutions().should('have.length.gte', 1);
            ...
            dataJobManageDetailsPage.getMainTitle().should('contain.text', testJob.job_name);
            ...
            dataJobManageDetailsPage.showMoreDescription().getDescriptionFull().should('contain.text', testJob.description);
        });
        ...
        it('Verify most recent failing executions Widget navigates correctly', () => {
            ...
            dataJobsHealthPanel.navigateToMostRecentFailingJobExecutions(testJob.job_name);

            const dataJobManageExecutionsPage = DataJobManageExecutionsPage.getPage();
            dataJobManageExecutionsPage.getMainTitle().should('contain.text', testJob.job_name);
            ...
            dataJobManageExecutionsPage.getDataGridRows().should('have.length.gte', 1);
        });
    });
});
