        ...
        describe('Methods::()', () => {
            describe('|register|', () => {
                it('should verify will execute register and execute methods in Registry instance', () => {
                    // Given
                    // @ts-ignore
                    const spyRegister = spyOn(SystemEventHandlerRegistry.instance, 'register').and.callThrough();
                    // @ts-ignore
                    const spyExecute = spyOn(SystemEventHandlerRegistry.instance, 'execute').and.callThrough();
                    const handler = () => Promise.resolve(true);
                    const handlerClassInstance = {};

                    // When
                    SystemEventHandlerRegistry.register('SE_Send_Email', handler, handlerClassInstance);

                    // Then
                    // @ts-ignore
                    expect(spyRegister).toHaveBeenCalledWith('SE_Send_Email', handler, handlerClassInstance, null);
                    // @ts-ignore
                    expect(spyExecute).toHaveBeenCalledWith('SE_Send_Email', handler, jasmine.any(Function), handlerClassInstance, null);
                });
            ...
            describe('|unregister|', () => {
                it('should verify will execute unregister and execute methods in Registry instance', () => {
                    // Given
                    // @ts-ignore
                    const spyUnregister = spyOn(SystemEventHandlerRegistry.instance, 'unregister').and.callThrough();
                    // @ts-ignore
                    const spyExecute = spyOn(SystemEventHandlerRegistry.instance, 'execute').and.callThrough();
                    ...
                    // @ts-ignore
                    expect(spyExecute).toHaveBeenCalledWith(event, handler, jasmine.any(Function));
                });
            ...
            describe('|findHandlerByReference|', () => {
                it('should verify will find Handler by its reference', () => {
                    ...
                    // When
                    SystemEventHandlerRegistry.register(event, handlerRef, clazzInstance);
                    const record = SystemEventHandlerRegistry.findHandlerByReference(event, handlerRef, clazzInstance);
            ...
            describe('|getPreparedArrayOfHandlers|', () => {
                it('should verify will return empty Array when there is no Handlers for Event', () => {
                    ...
                    // When
                    const handlers = SystemEventHandlerRegistry.getPreparedArrayOfHandlers(event);
                ...
                it('should verify will return 3 handlers for given Event', () => {
                    ...
                    SystemEventHandlerRegistry.register(event, handlerRef1, clazzInstance);
                    SystemEventHandlerRegistry.register(event, handlerRef2, clazzInstance);
                    SystemEventHandlerRegistry.register(event, handlerRef3, clazzInstance);
                    ...
                    // Then
                    expect(handlers).toEqual([
                        { handlerRef: handlerRef1, handlerClassInstance: clazzInstance, handlerFilterExpression: null },
                        { handlerRef: handlerRef2, handlerClassInstance: clazzInstance, handlerFilterExpression: null },
                        { handlerRef: handlerRef3, handlerClassInstance: clazzInstance, handlerFilterExpression: null }
                    ]);
                });
                ...
                it('should verify will return 3 handlers in reverse order for given Event', () => {
                    ...
                    SystemEventHandlerRegistry.register(event, handlerRef11, clazzInstance);
                    SystemEventHandlerRegistry.register(event, handlerRef22, clazzInstance);
                    SystemEventHandlerRegistry.register(event, handlerRef33, clazzInstance);

                    // When
                    const handlers = SystemEventHandlerRegistry.getPreparedArrayOfHandlers(event, true);

                    // Then
                    expect(handlers).toEqual([
                        { handlerRef: handlerRef33, handlerClassInstance: clazzInstance, handlerFilterExpression: null },
                        { handlerRef: handlerRef22, handlerClassInstance: clazzInstance, handlerFilterExpression: null },
                        { handlerRef: handlerRef11, handlerClassInstance: clazzInstance, handlerFilterExpression: null }
                    ]);
                });
            });
        });
