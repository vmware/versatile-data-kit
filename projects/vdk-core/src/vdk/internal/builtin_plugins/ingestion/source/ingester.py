...
from vdk.internal.builtin_plugins.ingestion.source.data_source import DataSourceError
...
class DataSourceIngester:
    ...
    def _ingest_stream(self, ingest_entry: IngestQueueEntry):
        for payload in ingest_entry.stream.read():
            ...
            if payload.data:
                destination_table = self._infer_destination_table(ingest_entry, payload)

                self.__actual_ingester.send_object_for_ingestion(
                    payload=payload.data,
                    destination_table=destination_table,
                    method=ingest_entry.method,
                    target=ingest_entry.target,
                    collection_id=ingest_entry.collection_id,
                )

    @staticmethod
    def _infer_destination_table(ingest_entry, payload):
        if ingest_entry.destination_table:
            destination_table = ingest_entry.destination_table
    ...
    def ingest_data_source(
        ...
        for stream in data_source.streams():
            entry = IngestQueueEntry(
                stream, destination_table, method, target, collection_id, error_callback
            )
