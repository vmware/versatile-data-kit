...
class IngestToTrino(IIngesterPlugin):
    ...
    def ingest_payload(
        ...
        with self.context.connections.open_connection(
            ...
            cur = conn.cursor()
            # self._check_destination_table_exists(destination_table, cur)
            self._ingest_payload(destination_table, payload, cur, batch_size)
    ...
    def _ingest_payload(
        ...
        for lower_bound, upper_bound in self._create_batching_indices(
    ...
    def _check_destination_table_exists(
    ...
    @staticmethod
    def _create_batching_indices(payload_size: int, batch_size: int):
        """
