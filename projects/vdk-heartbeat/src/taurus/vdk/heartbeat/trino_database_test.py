...
class TrinoDatabaseRunTest(DatabaseHeartbeatTest):
    ...
    def _python_type_to_trino_type(column_type: Type):
        ...
        if trino_type:
            ...
            log.warning(f"Cannot convert that column type : {column_type}. Will assume varchar"
    ...
    def _python_value_to_trino_value(column_value: Any):
        ...
        log.warning(f"Cannot convert that column type : {type(column_value)} of value {column_value}. Will assume varchar.")
        return f"'{column_value}'"
    ...
    def _insert_table_row(self, db: str, table: str, row: Dict[str, Any]):
        ...
        values = ",".join([self._python_value_to_trino_value(v) for _, v in row.items()])
    ...
    def _select_data(self, db: str, table: str, column_filters: Dict[str, str]) -> List[List]:
        where_columns = " and ".join([k + "=" + self._python_value_to_trino_value(v) for k, v in column_filters.items()])
    ...
    def _db_connection(self):
        ...
        auth = (
            BasicAuthentication(self.config.DATABASE_USER,
                                self.config.DATABASE_PASS) if self.config.DATABASE_PASS else None
        )
        ...
        conn = dbapi.connect(
            ...
            catalog=None,
            schema=None,
            http_scheme=constants.HTTPS if self.config.DATABASE_USE_SSL else constants.HTTP,
