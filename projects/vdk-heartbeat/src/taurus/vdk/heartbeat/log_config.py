...
def configure_loggers(app_name = 'vdk-heartbeat' , op_id=time.time(), log_config_type=None, vdk_logging_level='INFO'):
    """
    ...
    DETAILED_FORMAT = f'%(asctime)s10.0 [%(levelname)-5.5s] [{app_name}] %(name)-10.10s ' \
    ...
    if ('NONE' == log_config_type):
        ...
        LOCAL = {  # @UnusedVariable
            'version': 1,
            'handlers': {
                'consoleHandler': _CONSOLE_HANDLER
            },
