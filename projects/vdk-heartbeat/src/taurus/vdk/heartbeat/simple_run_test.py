        ...
        while time.time() - start_time < self.config.RUN_TEST_TIMEOUT_SECONDS:
            ...
            try:
                ...
                if props and 'now' in props:
                    log.info(f"Data Job has recorded successfully property 'now' = {props['now']}")
                    return
                else:
                    log.info(
                ...
                time.sleep(wait_time_seconds)
        if caught_exception:
            raise AssertionError("Simple test failed with exception. See cause for more.") from caught_exception
        else:
            raise AssertionError("Simple test failed with timeout. "
