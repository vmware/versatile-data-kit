...
class Config:
    ...
    def __init__(self, config_file=None):
        ...
        self.DATABASE_USE_SSL = self.get_value('DATABASE_USE_SSL', 'yes').lower() in ('yes', 'y', 'true', 1)
        self.DATABASE_VERIFY_SSL = self.get_value('DATABASE_VERIFY_SSL', 'yes').lower() in ('yes', 'y', 'true', 1)
        self.DATABASE_AUTHN_TYPE = self.get_value('DATABASE_AUTHENTICATION_MECHANISM', is_required=False)

        # Database test configuration.
        self.DATABASE_TEST_DB = self.get_value('DATABASE_TEST_DB', 'taurus_testing_sandbox')
        self.DATABASE_TEST_TABLE_SOURCE = self.get_value('DATABASE_TEST_TABLE_SOURCE',
                                                         f'vdk_heartbeat_source_{table_suffix}')
        self.DATABASE_TEST_TABLE_DESTINATION = self.get_value('DATABASE_TEST_TABLE_DESTINATION',
                                                              f'vdk_heartbeat_destination_{table_suffix}')
        self.DATABASE_TEST_TABLE_LOAD_DESTINATION = self.get_value('DATABASE_TEST_TABLE_LOAD_DESTINATION',
                                                                   f'vdk_heartbeat_load_destination_{table_suffix}')

        self.DATABASE_CONNECTION_TIMEOUT_SECONDS = int(self.get_value('DATABASE_CONNECTION_TIMEOUT_SECONDS', '180'))
        self.RUN_TEST_TIMEOUT_SECONDS = int(self.get_value('RUN_TEST_TIMEOUT_SECONDS', '1200'))
        ...
        self.job_name = self.get_value('JOB_NAME', f'vdk-heartbeat-data-job-{job_suffix}')
        self.job_version = self.get_value('JOB_VERSION', 'HEAD')
        self.job_team = self.get_value('JOB_TEAM', 'taurus')
        self.job_notification_mail = self.get_value('JOB_NOTIFICATION_MAIL', None, is_required=False)
        self.data_job_directory_parent = self.get_value("DATAJOB_DIRECTORY_PARENT", None, is_required=False)
        # by default possible options are: impala, trino, simple
        self.data_job_directory_name = self.get_value("DATAJOB_DIRECTORY_NAME", "impala")
        self.database_test_module_name = self.get_value("JOB_RUN_TEST_MODULE_NAME",
                                                        "taurus.vdk.heartbeat.impala_database_test")
        self.database_test_class_name = self.get_value("JOB_RUN_TEST_CLASS_NAME", "ImpalaDatabaseHeartbeatTest")
        self.db_default_type = self.get_value("DB_DEFAULT_TYPE", 'IMPALA')
        self.vdk_command_name = self.get_value("VDK_COMMAND_NAME", "vdkcli")

        self.clean_up_on_failure = self._string_to_bool(self.get_value("CLEAN_UP_ON_FAILURE", 'true', False))
        ...
        """
        self.report_junit_xml_file_path = self.get_value("REPORT_JUNIT_XML_FILE_PATH", "tests.xml")
