...
class JobController:
    ...
    def __init__(self, config: Config):
        self.config = config
        log.info(f"Using Control Service REST API URL: {config.control_api_url} "
    ...
    def _execute(self, command):
        ...
        try:
            ...
            log.exception(f"{full_command} failed. \nstdout: {process_error.stdout}, \nstderr: {process_error.stderr}")
            raise

    @LogDecorator(log)
    def login(self):
        self._execute(["login",
                       "-u", f"{self.config.vdkcli_oauth2_uri}",
                       "-a", f"{self.config.vdkcli_api_refresh_token}",
                       "-t", "api-token"])
    ...
    def check_list_jobs(self):
        res = self._execute(["list",
                             "-o", "json",
                             "-u", self.config.control_api_url,
                             "-t", self.config.job_team])
        res = json.loads(res)
        assert filter(lambda j: j['job_name'] == self.config.job_name, res), \
            f"list command missing created job {self.config.job_name}"

    @LogDecorator(log)
    def show_job_details(self):
        res = self._execute(["show",
                             "-u", self.config.control_api_url,
                             "-o", "json",
                             "-n", self.config.job_name,
                             "-t", self.config.job_team])
        res = json.loads(res)
        log.info(f"Team {self.config.job_team}, Job: {self.config.job_name} details: {json.dumps(res, indent=2)}")
    ...
    def check_deployments(self, enabled=True, timeout_seconds=600):
        ...
        while not deployments and time.time() - start < timeout_seconds:
            deployments = self._execute(["deploy", "--show",
                                         "-o", "json",
                                         "-u", self.config.control_api_url,
                                         "-n", self.config.job_name,
                                         "-t", self.config.job_team])
    ...
    def _update_config_ini(self, heartbeat_job_dir):
        ...
        if self.config.job_notification_mail:
            ...
            config_ini.set('contacts', 'notified_on_job_success', self.config.job_notification_mail)
            config_ini.set('contacts', 'notified_on_job_deploy', self.config.job_notification_mail)
            config_ini.set('contacts', 'notified_on_job_failure_user_error', self.config.job_notification_mail)
            config_ini.set('contacts', 'notified_on_job_failure_platform_error', self.config.job_notification_mail)
    ...
    def _get_data_job_dir(self, tmpdir):
        if self.config.data_job_directory_parent:
            ...
            vdk_heartbeat_data_job_path = taurus.vdk.vdk_heartbeat_data_job.__path__._path[0]
        source_data_job_directory = os.path.abspath(
            os.path.join(vdk_heartbeat_data_job_path, self.config.data_job_directory_name))
...
def run(job_input):
    ...
    job_input.set_all_properties(props)
        ...
        with open(os.path.join(heartbeat_job_dir, '06_override_properties.py'), 'w') as pyfile:
