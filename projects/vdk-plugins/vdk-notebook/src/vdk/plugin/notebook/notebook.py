...
from vdk.plugin.notebook.cell import CellFunFactory
...
class Notebook:
        ...
        try:
            ...
            for jupyter_cell in content["cells"]:
                if jupyter_cell["cell_type"] == "code":
                    ...
                    if CellFunFactory.is_vdk_cell(cell):
                        if CellFunFactory.is_sql_cell(cell) or CellFunFactory.is_vdk_run_cell(cell):
    ...
    def register_notebook_steps(self, context: JobContext):
        ...
        for index, cell in enumerate(self.sql_and_run_cells):
            cell_type = TYPE_PYTHON if CellFunFactory.is_vdk_run_cell(cell) else TYPE_SQL
            runner_func = (
            ...
            if cell_type == TYPE_PYTHON:
                CellFunFactory.combine_cells(cell, self.python_helper_cells)
            step = NotebookStep(
                name="".join(
                    [
                ...
                code=CellFunFactory.get_cell_code(cell),
            )
