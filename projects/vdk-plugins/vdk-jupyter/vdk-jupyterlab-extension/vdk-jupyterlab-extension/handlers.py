    ...
    def post(self):
        ...
        try:
            status = VdkUI.download_job(
                input_data["jobName"], input_data["jobTeam"], input_data["restApiUrl"], input_data["parentPath"]
            )
            self.finish(json.dumps({"message": f"{status}"}))
        except Exception as e:
            self.finish(json.dumps({"message": f"{e}"}))
...
def setup_handlers(web_app):
    ...
    download_job_route_pattern = url_path_join(base_url, "vdk-jupyterlab-extension", "download")
