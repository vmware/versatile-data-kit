...
export async function getTokenFromLocation(
  ...
  if (location.storageType === 'localStorage') {
    // TODO: window.top.localStorage is likely to be forbidden in most setups due to cross-origin resitrictions
    // Consider using window.top.postMessage and eventListener.
    let item = safeJSONParse(window.localStorage.getItem(location.key));
