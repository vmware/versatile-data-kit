...
def is_impala_error(received_exception):
...
def _is_query_memory_error(exception):
    return errors.exception_matches(
        exception,
        classname_with_package="impala.error.OperationalError",
...
def _is_redshift_error(exception):
    return errors.exception_matches(
        exception,
        classname_with_package="psycopg2.*",
