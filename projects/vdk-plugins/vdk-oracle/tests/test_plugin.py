...
class OracleTests(TestCase):
    ...
    def test_oracle_ingest_job_case_insensitive_neg_1(self):
        runner = CliEntryBasedTestRunner(oracle_plugin)
        result: Result = runner.invoke(
            [
                "run",
                jobs_path_from_caller_directory(
                    "oracle-ingest-job-case-insensitive-neg1"
                ),
            ]
        )
        cli_assert_equal(0, result)
        _verify_ingest_wrong_case(runner)

    def test_oracle_ingest_job_case_insensitive_neg_2(self):
        runner = CliEntryBasedTestRunner(oracle_plugin)
        result: Result = runner.invoke(
            [
                "run",
                jobs_path_from_caller_directory(
                    "oracle-ingest-job-case-insensitive-neg2"
                ),
            ]
        )
        cli_assert_equal(1, result)
        assert (
            'Exception was: ORA-01400: cannot insert NULL into ("SYSTEM"."ORACLE_INGEST_CASE_INSENSITIVE_NEG_TWO"."ID")'
            in result.output
        )
...
def _verify_ingest_wrong_case(runner):
    check_result = runner.invoke(
        ["sql-query", "--query", "SELECT * FROM oracle_ingest_case_insensitive_neg_one"]
    )
    expected = (
        "ID    STR_DATA    INT_DATA    FLOAT_DATA    BOOL_DATA    TIMESTAMP_DATA\n"
        "----  ----------  ----------  ------------  -----------  ----------------\n"
