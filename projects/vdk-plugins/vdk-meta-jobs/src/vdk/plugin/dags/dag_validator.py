...
allowed_job_keys = {"job_name", "team_name", "fail_dag_on_error", "depends_on"}
...
class DagValidator:
    ...
    def _validate_job(self, job: Dict):
        ...
        self._validate_fail_dag_on_error(job)
    ...
    def _validate_fail_dag_on_error(self, job: Dict):
        if "fail_dag_on_error" in job and not isinstance(
            (job["fail_dag_on_error"]), bool
        ):
            self._raise_error(
                list(job),
                ERROR.TYPE,
                ...
                f" is {type(job['fail_dag_on_error'])}. Expected type is bool.",
            )
