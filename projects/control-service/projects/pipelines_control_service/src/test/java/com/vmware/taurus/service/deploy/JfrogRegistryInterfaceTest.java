...
class JfrogRegistryInterfaceTest {
    ...
    @Mock
    private ArtifactoryClientBuilder artifactoryClientBuilder;
    ...
    @InjectMocks
    private JfrogRegistryInterface jfrogRegistryInterface;
    ...
    void testCheckJfrogImageExists_ImageExists() {
        try (MockedStatic<ArtifactoryClientBuilder> artifactoryClientBuilderMockedStatic = mockStatic(ArtifactoryClientBuilder.class)) {
            artifactoryClientBuilderMockedStatic.when(ArtifactoryClientBuilder::create).thenReturn(artifactoryClientBuilder);
            List<AqlItem> repoPaths = Collections.singletonList(mock(AqlItem.class));
            when(artifactory.searches().repositories(anyString()).artifactsByFileSpec(any(FileSpec.class))).thenReturn(repoPaths);
    ...
    void testCheckJfrogImageExists_ImageDoesNotExist() {
        try (MockedStatic<ArtifactoryClientBuilder> artifactoryClientBuilderMockedStatic = mockStatic(ArtifactoryClientBuilder.class)) {
            artifactoryClientBuilderMockedStatic.when(ArtifactoryClientBuilder::create).thenReturn(artifactoryClientBuilder);
            // Mocking behavior
            when(searches.repositories(anyString()).artifactsByFileSpec(any(FileSpec.class))).thenReturn(Collections.emptyList());
    ...
    void testCheckJfrogImageExists_Exception() {
        try (MockedStatic<ArtifactoryClientBuilder> artifactoryClientBuilderMockedStatic = mockStatic(ArtifactoryClientBuilder.class)) {
            artifactoryClientBuilderMockedStatic.when(ArtifactoryClientBuilder::create).thenReturn(artifactoryClientBuilder);
            // Mocking behavior
            when(artifactory.searches().repositories(anyString()).artifactsByFileSpec(any(FileSpec.class))).thenThrow(new RuntimeException("Simulated exception"));
