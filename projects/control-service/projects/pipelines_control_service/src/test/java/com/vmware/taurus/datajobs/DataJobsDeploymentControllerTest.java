...
public class DataJobsDeploymentControllerTest {
    ...
    @Test
    public void whenKubernetesIsUnavailableTheControlPlaneShouldReturnAnErrorInsteadOfSayingNoJobs() throws Exception {
        ...
        mockMvc.perform(post("/data-jobs/for-team/team-name/jobs")
                        .contentType(MediaType.APPLICATION_JSON)
                .content(new ObjectMapper()
                .writeValueAsString(new DataJob("job-name","desc" ,
                        new DataJobConfig())))).andExpect(status().is(201));
        mockMvc.perform(get("/data-jobs/for-team/team-name/jobs/job-name/deployments")).andExpect(status().is(503));
    }
