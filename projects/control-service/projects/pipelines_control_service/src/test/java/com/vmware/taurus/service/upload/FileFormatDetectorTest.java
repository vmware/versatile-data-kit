...
class FileFormatDetectorTest {
    ...
    void test_matchTypes_errors() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> new FileFormatDetector().matchTypes("", ""));
        Assertions.assertThrows(IllegalArgumentException.class, () -> new FileFormatDetector().matchTypes("type", "type"));
        Assertions.assertThrows(IllegalArgumentException.class, () -> new FileFormatDetector().matchTypes("type", "type/subtype"));
    }

    @Test
    void test_DetectType() throws IOException {
        var detector = new FileFormatDetector();
        Assertions.assertEquals("application/json", detector.detectFileType(getResource("detect.json")));
