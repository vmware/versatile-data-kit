...
public class JobExecutionRepositoryIT {
   ...
   @Test
   void testFindFirstByDataJobNameAndStatusInOrderByEndTimeDesc_withNoExecutions_shouldReturnEmpty() {
      DataJob dataJob = RepositoryUtil.createDataJob(jobsRepository);

      var lastExecution = jobExecutionRepository.findFirstByDataJobNameAndStatusInOrderByEndTimeDesc(dataJob.getName(),
              List.of(ExecutionStatus.RUNNING, ExecutionStatus.FINISHED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED));

      Assertions.assertTrue(lastExecution.isEmpty());
   }

   @Test
   void testFindFirstByDataJobNameAndStatusInOrderByEndTimeDesc_withNoCompletedExecutions_shouldReturnEmpty() {
      DataJob dataJob = RepositoryUtil.createDataJob(jobsRepository);
   ...
   @Test
   void testFindFirstByDataJobNameAndStatusInOrderByEndTimeDesc_shouldReturnTheLastExecutionWithStatus() {
      DataJob dataJob = RepositoryUtil.createDataJob(jobsRepository);
      ...
      Assertions.assertTrue(lastExecution.isPresent());
      Assertions.assertEquals("execution3", lastExecution.get().getId());
   }
}
