...
class JobFieldStrategyByJobPythonVersionTest {
    ...
    void testJobDescriptionStrategy_whenGettingStrategyName_shouldBeSpecific() {
        assertThat(strategyByJobPythonVersion.getStrategyName()).isEqualTo(JobFieldStrategyBy.DEPLOYMENT_JOBPYTHONVERSION);
    }
    ...
    void testJobDescriptionStrategy_whenAlteringFieldData_shouldNotModifyState() {
        ...
        assertThat(dataJob.getDeployments().stream().findFirst().orElseThrow().getJobPythonVersion()).isEqualTo("3.9");
    }
    ...
    public void testJobDescriptionComparator() {
        ...
        var jobPythonVersionCriteria = strategyByJobPythonVersion.computeFilterCriteria(baseCriteria, filter);
