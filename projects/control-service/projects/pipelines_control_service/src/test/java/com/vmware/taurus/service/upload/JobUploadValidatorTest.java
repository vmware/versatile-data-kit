...
class JobUploadValidatorTest {
    ...
    /**
     * The testing job has the following structure:
     * ├── config.ini
     * ├── package
     * │   ├── binary
     * │   ├── lib.py
    ...
    void test_validateJobAllowed_base_types() throws IOException {
        JobUploadValidator validator = new JobUploadValidator(new String[]{"text", "application/octet-stream"});
        validator.validateJob("foo", getTestJob());
    }

    @Test
    void test_validateJobAllowed_full_types() throws IOException {
        JobUploadValidator validator = new JobUploadValidator(new String[]{"application/octet-stream", "text/x-ini", "text/x-sql", "text/x-python"});
        validator.validateJob("foo", getTestJob());
    }

    @Test
    void test_validateJobAllowed_not_allowed_sql_extension() {
        JobUploadValidator validator = new JobUploadValidator(new String[]{"application/octet-stream", "text/x-ini",  "text/x-python"});
        Assertions.assertThrows(InvalidJobUpload.class, () -> validator.validateJob("foo", getTestJob()));
