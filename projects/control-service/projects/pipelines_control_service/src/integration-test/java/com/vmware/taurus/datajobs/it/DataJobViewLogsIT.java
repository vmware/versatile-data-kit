  ...
  public void testJobLogsViewing(
    ...
    Awaitility.await().atMost(6, TimeUnit.MINUTES)
            ...
            .ignoreExceptionsMatching((e) ->
                    // It is only by looking 3 exceptions deep that we can tell the reason the logs read failed.
                    e.getCause().getCause()
                    // If 400 is in the error message then the pod is not up yet and we should keep trying to poll for the logs.
     ...
     */
  private void getLogsFast(String jobName, String teamName, String executionId) throws InterruptedException, ExecutionException, TimeoutException {
