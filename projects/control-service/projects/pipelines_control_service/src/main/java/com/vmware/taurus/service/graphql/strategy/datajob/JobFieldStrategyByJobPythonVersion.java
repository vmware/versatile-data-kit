...
public class JobFieldStrategyByJobPythonVersion extends FieldStrategy<V2DataJob> {
    ...
    private static final Comparator<V2DataJob> COMPARATOR_DEFAULT =
            Comparator.comparing(
                    e -> e.getDeployments().stream().findFirst().orElseThrow().getJobPythonVersion(), Comparator.nullsLast(Comparator.naturalOrder()));
                    ...
                    predicate.and(
                            dataJob ->
                                    dataJob.getDeployments() != null
                                            ...
                                            dataJob.getDeployments().stream().findFirst().orElseThrow().getJobPythonVersion(), filter.getPattern()));
        }
    ...
    public Predicate<V2DataJob> computeSearchCriteria(@NonNull String searchStr) {
        return dataJob -> {
            ...
            return StringUtils.containsIgnoreCase(dataJob.getDeployments().stream().findFirst().orElseThrow().getJobPythonVersion(), searchStr);
        };
    }
