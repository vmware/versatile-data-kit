...
public abstract class KubernetesService {
  ...
  public Pair<Boolean, String> health() {
    try {
      var info = new VersionApi(client).getCode().execute();
  ...
  public void deleteNamespace(String namespaceName) throws ApiException {
    try {
      new CoreV1Api(client).deleteNamespace(namespaceName).execute();
  ...
  public Set<String> listJobs() throws ApiException {
    ...
    var jobs = batchV1Api.listNamespacedJob(namespace).execute();
    var set =
  ...
  public Optional<JobDeploymentStatus> readV1CronJob(String cronJobName) {
    ...
    try {
      cronJob = batchV1Api.readNamespacedCronJob(cronJobName, namespace).execute();
  ...
  public List<JobDeploymentStatus> readV1CronJobDeploymentStatuses() {
    ...
    try {
      cronJobs = batchV1Api.listNamespacedCronJob(namespace).execute();
    } catch (ApiException e) {
  ...
  private void startNewV1CronJobExecution(
      ...
      String jobName)
      throws ApiException {
    var cron = batchV1Api.readNamespacedCronJob(cronJobName, namespace).execute();
  ...
  public void createV1CronJob(
    ...
    var cronJob =
        v1CronJobFromTemplate(
            ...
            jobLabels,
            imagePullSecrets);
    V1CronJob nsJob = batchV1Api.createNamespacedCronJob(namespace, cronJob).execute();
  ...
  public void updateV1CronJob(
    ...
    var cronJob =
        v1CronJobFromTemplate(
            ...
            jobLabels,
            imagePullSecrets);
    V1CronJob nsJob = batchV1Api.replaceNamespacedCronJob(name, namespace, cronJob).execute();
  ...
  public void createNewJob(
    ...
    V1Job nsJob = batchV1Api.createNamespacedJob(namespace, job).execute();
  ...
  public Optional<V1Pod> getPod(String podName) throws ApiException {
    List<V1Pod> allPods = new CoreV1Api(client).listNamespacedPod(namespace).execute().getItems();
  ...
  public boolean isRunningJob(String dataJobName) throws ApiException {
    ...
    String labelSelector = buildLabelSelector(labelsToSelect);
    V1JobList v1JobList =
        batchV1Api.listNamespacedJob(namespace).labelSelector(labelSelector).execute();
   ...
   */
  private String readNamespacedPodLog(
  ...
  public void watchJobs(
    ...
    try {
      var jobList = batchV1Api.listNamespacedJob(namespace).labelSelector(labelSelector).execute();
  ...
  public void deleteJob(String name) throws ApiException {
    ...
    try {
      var status = batchV1Api.deleteNamespacedJob(name, namespace).execute();
  ...
  private Optional<V1Job> getJob(String jobName) throws ApiException {
    String fieldSelector = String.format("metadata.name=%s", jobName);
    try {
      var jobs = batchV1Api.listNamespacedJob(namespace).fieldSelector(fieldSelector).execute();
  ...
  public Map<String, byte[]> getSecretData(String name) throws ApiException {
    ...
    try {
      V1Secret v1Secret = api.readNamespacedSecret(name, this.namespace).execute();
  ...
  public void removeSecretData(String name) throws ApiException {
    ...
    try {
      var status = new CoreV1Api(client).deleteNamespacedSecret(name, this.namespace).execute();
  ...
  public void saveSecretData(String name, Map<String, byte[]> data) throws ApiException {
    ...
    V1Secret nsSecret;
    try {
      nsSecret = api.replaceNamespacedSecret(name, this.namespace, secret).execute();
    } catch (ApiException e) {
      if (e.getCode() == 404) {
        log.debug("Secret {} does not exist. Creating ...", name);
        nsSecret = api.createNamespacedSecret(this.namespace, secret).execute();
      } else {
