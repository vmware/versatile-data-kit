...
public class ServiceApp {
    ...
    @Bean
    OpenAPI apiInfo() {
        return (new OpenAPI()).info((new Info().title("Versatile Data Kit Control Service API")
        ...
        ).license((new License()).name("Apache 2.0").url("https://www.apache.org/licenses/LICENSE-2.0.html")).version("1.0")).components((new Components()).addSecuritySchemes("bearerAuth", (new SecurityScheme()).type(SecurityScheme.Type.HTTP).scheme("bearer").bearerFormat("JWT")));
    }
