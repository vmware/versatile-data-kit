...
public class DeploymentService {
    ...
    public JobDeploymentStatus readDeployment(String jobName) {
        return jobImageDeployer.readScheduledJob(jobName).orElseThrow(() -> new DataJobDeploymentNotFoundException(jobName));
    }

    public boolean deploymentExists(String jobName) {
        return jobImageDeployer.deploymentExists(jobName);
    }
    ...
    public void patchDeployment(DataJob dataJob, JobDeployment jobDeployment) {
        ...
        var oldDeployment =
                DeploymentModelConverter.toJobDeployment(
                        dataJob.getJobConfig().getTeam(), dataJob.getName(), deploymentStatus);
    ...
    public void updateDeployment(
        ...
        try {
            ...
            if (deploymentExists(dataJob.getName())) {
                var deploymentStatus = readDeployment(dataJob.getName());
                var oldDeployment =
                        DeploymentModelConverter.toJobDeployment(
                                dataJob.getJobConfig().getTeam(), dataJob.getName(), deploymentStatus);
