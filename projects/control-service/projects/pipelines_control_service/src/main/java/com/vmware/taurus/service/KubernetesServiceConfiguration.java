...
public class KubernetesServiceConfiguration {
    ...
    @Bean
    public BatchV1Api deploymentBatchV1Api(@Qualifier("deploymentApiClient") ApiClient deploymentApiClient) {
        return new BatchV1Api(deploymentApiClient);
    }

    @Bean
    public BatchV1beta1Api deploymentBatchV1beta1Api(@Qualifier("deploymentApiClient") ApiClient deploymentApiClient) {
        return new BatchV1beta1Api(deploymentApiClient);
    }

    @Bean
    public ApiClient deploymentApiClient(@Value("${datajobs.deployment.k8s.kubeconfig:}") String kubeconfig) throws Exception {
        return getClient(kubeconfig);
    }

    @Bean
    public ApiClient controlApiClient(@Value("${datajobs.control.k8s.kubeconfig:}") String kubeconfig) throws Exception {
        return getClient(kubeconfig);
    }
    ...
    @Bean
    public BatchV1beta1Api controlBatchV1beta1Api(@Qualifier("controlApiClient") ApiClient apiClient) {
