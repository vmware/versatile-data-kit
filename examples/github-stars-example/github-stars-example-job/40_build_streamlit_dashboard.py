...
db_connection = sqlite3.connect("/tmp/vdk-sqlite.db")

# Fetch data
df = pd.read_sql_query(f"SELECT * FROM github_star_history", db_connection)
...
chart = px.line(
    ...
    x="starred_date",
    y="count",
    color_discrete_sequence=["#C996CC"],
    labels={"starred_date": "Date", "count": "Star Count"},
)
